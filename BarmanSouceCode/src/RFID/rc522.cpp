...
const uint8_t tarjetamariano[]={0x6b, 0x3c, 0x0f, 0x1c, 0x44};
const uint8_t llaveritomariano[]={0x7a, 0x86, 0x2f, 0x16, 0xc5};
const uint8_t tarjeta[]={0xc3, 0x6c, 0x1a, 0x02, 0xb7};
const uint8_t llaverito[]={0xa3, 0x42, 0x94, 0x14, 0x61};
...
void convert_to_hex_format(const uint8_t* input, uint8_t* output) {
    // Procesamos cada uno de los 5 caracteres de entrada
    for (int i = 0; i < 5; i++) {
        // Almacenamos cada carácter en formato hexadecimal en el formato %XX
        //*output++ = '%';
        unsigned char byte = (unsigned char)input[i];
        *output++ = "0123456789ABCDEF"[byte >> 4];   // Dígito alto en hexadecimal
        *output++ = "0123456789ABCDEF"[byte & 0x0F]; // Dígito bajo en hexadecimal
    }
    *output = '\0';  // Terminador nulo para la cadena resultante
}

uint8_t showSerialNumber(uint8_t* cardId)
{
	uint8_t serNum[16];	//ojo que necesito el doble para guardar los caracteres en Hexa
	uint8_t flag_encontrado = 0;
	uint8_t status;
	uint8_t str[MAX_LEN];

	// 4 bytes serial num de la tarjeta, el 5to byte es la verificacion ( XOR entre los bytes 1-4 )

	setup();											// inicializa el  RFID Reader

	status = MFRC522_Request(PICC_REQIDL, str);			//Search card, return card types

	status = MFRC522_Anticoll(str);						//Prevent conflict, return the 4 bytes Serial number of the card
//	for(i=0; i<5; i++)
//		serNum[i]=str[i];
	convert_to_hex_format(str,serNum);			// convierte SerNum en un solo str

	serNum[15] = '\0';
	if (status == MI_OK)
	{
		//if( !strncmp( (char *)tarjeta, (char *)str, 5) || !strncmp( (char *)llaverito, (char *)str, 5) )
		//{
			//sprintf((char *)serNum,"%x%x%x%x%x",str[0],str[1],str[2],str[3], str[4]);

			strncpy(cardId,serNum,16);// Reconoce ID, lo guarda en serNum, lo envia al ESP32 y a DB. Inicializa MdE buscarBebida
			//porfavor quietemos esto de aca
			flag_encontrado = 1;

		//}
	}
	MFRC522_Halt();
	return flag_encontrado;
...
void setup(void)
{
	nss.Set(OFF);			//nss = 0;						//Activate the RFID reader
	nrst.Set(ON);			//nrst=1;						//digitalWrite(NRSTPD,HIGH);
	MFRC522_Init();
}
...
int8_t readDataASCIIsectorBlock( uint8_t sector, uint8_t block, uint8_t line)
{
	//4 bytes Serial number of card, the 5 bytes is verfiy bytes
		...
		sprintf((char *)codigo, "%s", (char *)str);
